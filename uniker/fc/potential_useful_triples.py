...
if __name__ == '__main__':
    ...
    # dataset = sys.argv[1]
    data_path = '../data/kinship' # dataset path
    train_path = '../data/kinship/train.txt'
    hidden_triples_path = '../models/kinship/'
    model_path = '../models/kinship/' # path for trained model
    rule_name = 'MLN_rule.txt'
    use_cuda = True
    cuda = 2
    model = SelectHiddenTriples(data_path, train_path, hidden_triples_path, rule_name)

    model.run(threshold=50) # find all hidden triple # threshold (50)：randomly select 50 row and col
    # end_time = time.time()
